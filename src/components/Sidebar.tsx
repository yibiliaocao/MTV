...
declare global { interface Window { __sidebarCollapsed?: boolean; __RUNTIME_CUSTOM_CATEGORIES?: any[] } }
...
const Sidebar = ({ onToggle, activePath = '/' }: SidebarProps) => {
  ...
  useLayoutEffect(() => {
    ...
    if (saved !== null) { setIsCollapsed(JSON.parse(saved)); window.__sidebarCollapsed = JSON.parse(saved); }
  }, []);
  ...
  useEffect(() => {
    ...
    else setActive(searchParams.toString() ? `${pathname}?${searchParams.toString()}` : pathname);
  ...
  useEffect(() => {
    ...
    if (runtimeConfig?.CUSTOM_PARENT_CATEGORY?.length > 0) {
      const customMenu = runtimeConfig.CUSTOM_PARENT_CATEGORY.map(parent => ({
        icon: Star,
        label: parent.name,
  ...
  return (
    <SidebarContext.Provider value={contextValue}>
      <div className='hidden md:flex'>
        <aside data-sidebar className={`fixed top-0 left-0 h-screen bg-white/40 backdrop-blur-xl transition-all duration-300 border-r border-gray-200/50 z-10 shadow-lg dark:bg-gray-900/70 dark:border-gray-700/50 ${isCollapsed ? 'w-16' : 'w-64'}`} style={{ backdropFilter: 'blur(20px)', WebkitBackdropFilter: 'blur(20px)' }}>
                ...
                {menuItems.map(item => {
                  ...
                  return (
                    <Link key={item.label} href={item.href} onClick={() => setActive(item.href)} data-active={isActive} className='group flex items-center rounded-lg px-2 py-2 pl-4 text-sm text-gray-700 hover:bg-gray-100/30 hover:text-green-600 data-[active=true]:bg-green-500/20 data-[active=true]:text-green-700 transition-colors duration-200 min-h-[40px] dark:text-gray-300 dark:hover:text-green-400 dark:data-[active=true]:bg-green-500/10 dark:data-[active=true]:text-green-400 gap-3 justify-start'>
                      <div className='w-4 h-4 flex items-center justify-center'><Icon className='h-4 w-4' /></div>
